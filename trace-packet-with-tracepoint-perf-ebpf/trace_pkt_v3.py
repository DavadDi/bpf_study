...
def event_printer(cpu, data, size):
    ...
    if icmq_seq != event.icmpseq:
        icmq_seq = event.icmpseq
    ...
    delta_ms = (float(event.ts_us) - start_ts) / 1000
    ...
    fun_name = tps_name[event.fun_idx]
         ...
         # Print event
    print "%7s [%-12s] [%6s] %20s %4s %7s %7s %-34s" % (event.icmpseq, delta_ms, event.cpu, fun_name, event.ifname, event.icmpid, direction, flow)
